...
This guide covers deployment and onboarding for the Platform Escalation Service, its
Platform SRE Agent, and the optional reference Workload SRE Agent.
...
them automatically via a local `scripts/.deploy-state.json` file, with no copy/pasting
...
Routine proxy deployments preserve the current caller policy. See the
[escalation proxy operator runbook](../escalation-proxy/README.md) for complete health
checks, expected lifecycle states, and recovery guidance.
...
Only custom-agent YAML files are uploaded. The
[ALZ hub-and-spoke playbook](alz-hub-spoke-playbook.md) is never read or deployed by these
steps. To adopt that optional playbook, first merge its applicable guidance into the
existing liaison YAML; publishing `workload_liaison` then updates the existing agent
definition with the full combined prompt.
...
1. Open the SRE Agent in the Azure portal, then **Agent builder** > **Subagents** > **Create**.
...
## Known limitations and deployment caveats
...
- **SRE Agent Administrator role GUID** is resolved automatically at deploy time
...
- **Custom agent upload** (`Publish-CustomAgentYaml`) retries on `401/403` (RBAC
