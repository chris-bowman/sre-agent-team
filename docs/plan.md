...
- Current work package: WP9-WP12 remediation deployed; WP13 staging sign-off remains open
...
- [x] P12.1 Make readiness check bounded platform authentication and non-mutating connectivity, registry availability, acceptable policy freshness, and relevant circuit state. Dependency checks run through the bounded SDK offloader and are coalesced under one async lock with a configurable five-second per-replica cache in production. Liveness remains independent of dependency outages.
