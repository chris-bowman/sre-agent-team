...
- Current work package: deployable post-assessment candidate; deterministic Table concurrency and timeout-after-commit recovery regressions are deployed, while the remaining authorized private-Table P13.3 scenarios and operator sign-off remain open
...
- On 2026-09-15, an authorized controlled platform-dependency outage set a temporary invalid platform endpoint and a one-failure circuit threshold. A WorkloadApp create request returned only `503 Platform SRE Agent is temporarily unavailable`; liveness and MCP remained `200`, and readiness returned `503` while the circuit was open. The valid endpoint and normal threshold were restored in a new revision; liveness, readiness, and MCP all recovered to `200` without manual restart. No internal endpoint, exception, or credential detail was exposed.
- On 2026-09-15, a quota-one staging attempt revealed that expiry cleanup alone does not release a completed platform thread when its caller stops polling; the active metadata can otherwise consume quota until expiry. A new bounded terminal reconciler now conditionally leases active Table records across replicas, checks platform terminal state, and atomically releases quota for confirmed terminal records without reading report bodies. P10.6 and P13.3 require live private-Table verification of this new path.
- On 2026-09-16, commits `1dcbbc6` and `5a1b37a` added deterministic Table quota-one concurrent-writer coverage, concurrent readiness-burst coalescing coverage, and recovery for a transaction response timeout after a verified committed reservation. The deployed candidate revision `sre-escalation-proxy--0000029` runs two healthy replicas using immutable digest `sha256:75ce9734a2aeaa1bb1cdad0ff41de8ae5a71af92de3c8c8ae223d08f2aacbd51`. Twelve readiness samples returned `200` in 1.04-1.65 seconds against a 20-second probe timeout, and recent proxy logs contained no readiness, policy, reconciliation, exception, or traceback failures. Full local validation passed with 141 tests; the two named post-assessment CI selectors passed 34 and 38 tests. Trivy scanned the exact deployed digest with all HIGH/CRITICAL and unfixed findings included, reporting zero vulnerabilities. These results satisfy the automated candidate gate but do not replace the remaining P13.3 authorization-dependent staging matrix or P13.5 operator approval.
- On 2026-09-16, candidate revision `sre-escalation-proxy--0000030` deployed bounded uncertain-reservation recovery at immutable digest `sha256:43a16f12b029dd1462f4ab9ef075b909f71b553eadb063881b4911d3b5f45ac9`. The no-poll terminal reconciler scenario passed: WorkloadApp investigation `eac6c7a7-1203-431e-890a-ca2f2fae945d` was admitted under a temporary quota of one, intentionally received no caller status or summary access, was observed running on four one-minute worker sweeps, and was marked completed by `investigation_reconciled` at 02:37:43 UTC. A second investigation, `e9c58ed3-da3e-44ed-b5e3-1b8034b0bb38`, was admitted under the same quota, proving automatic terminal release and slot reuse. The quota was restored to ten with an ETag-protected private App Configuration update.
- On 2026-09-16, the unexpired-token revocation scenario passed for WorkloadIsolation. Its policy was disabled in private App Configuration while its `EscalationCaller` Entra assignment remained unchanged; after a fresh policy snapshot, the isolated caller received the expected safe `403` caller-policy denial. The policy was re-enabled through an ETag-protected update, and WorkloadIsolation investigation `3f6027af-67c7-4e26-ad96-2ec27721be99` was admitted, proving recovery without a role or runtime restart.
- On 2026-09-16, the final cross-caller read-isolation retest passed on revision `sre-escalation-proxy--0000032`: WorkloadApp status and summary requests for WorkloadIsolation investigation `3f6027af-67c7-4e26-ad96-2ec27721be99` both returned exactly `404 Investigation not found`. The retest exposed and corrected inconsistent status and summary error mappings in commits `f8520ac` and `e322caa`; neither transport now discloses foreign investigation state, findings, or identifiers.
- On 2026-09-16, WorkloadApp idempotency validation passed: initial create returned `d1d360bc-faf9-4ab0-bafa-c9d33ed42b60`, an identical retry with the same key replayed the same ID, and reuse of that key with only the description changed returned `409 Idempotency key was used with a different request`. This provides current staging evidence that a caller receives one logical investigation per key/fingerprint and cannot use an existing key for a changed request.
- On 2026-09-16, rolling restart survival passed on the current candidate: WorkloadApp investigation `77401afb-f225-4eee-847d-8baf14aec626` was created before a scoped revision-suffix update. Replacement revision `sre-escalation-proxy--p13restart20260916153118` reached two healthy replicas using the unchanged immutable digest `sha256:6ceb7068994ecfe03c6b5c52d63ef069251c65764703be1118f03661cb926dc5`; liveness, readiness, and MCP remained `200`. The original ID then completed and returned caller-safe `APPLICATION ISSUE` guidance, proving Table-backed state survived the rolling restart.
- On 2026-09-16, WorkloadApp submitted an OData-style idempotency key containing `p13-injection' or caller_appid eq 'a413478f-1cda-4eb6-aa72-794b91fde330`. The proxy treated it as literal data and returned only a new WorkloadApp investigation `2bb20fbe-fb3d-44bc-9603-0977594518bc`; no WorkloadIsolation identifier, state, findings, or other foreign data was disclosed.
- On 2026-09-16, controlled platform dependency outage/recovery passed on the candidate. An outage-only revision set an invalid HTTPS platform endpoint and circuit threshold one; a WorkloadApp create returned only safe `503 Platform SRE Agent is temporarily unavailable`. The restoration update completed despite the local Azure CLI connection resetting during polling: direct ARM inspection confirmed the latest-ready `p13recovery20260916160503` revision restored the original platform endpoint and threshold five, and liveness, readiness, and MCP each returned `200`. The request is retained as an uncertain no-thread outcome for the separate bounded-recovery verification rather than being represented as a definitely side-effect-free failed create.
- On 2026-09-17, a malformed-final Markdown fixture experiment verified explicit proxy routing through `/agent workload_liaison_p13_malformed_fixture`, but Azure SRE left the autonomous fixture task at `0 completed, 0 failed`; it cannot currently provide deterministic hosted fixture output. The temporary fixture was deleted and the proxy restored to `workload_liaison`. Commit `5d762c8` makes the configured liaison dispatch explicit, and the HTTP-level invalid-final Markdown regression verifies safe `502 invalid_platform_response` with no report, marker, or invalid-verdict disclosure. Treat the platform runtime limitation as residual staging-test infrastructure risk, not a passed hosted fixture test.
...
- [ ] P10.6 Add real-SDK-shaped entity and transaction-error tests, deterministic concurrent-writer barriers, fault injection, and an explicitly authorized isolated two-replica Table test. Deterministic quota-one concurrent-writer and timeout-after-commit recovery tests now pass; live private-Table verification and remaining crash/finalization/cleanup-race cases remain open.
...
- [x] P13.2 Rescan the exact candidate image digest, including unfixed vulnerabilities and applicable platforms. The Debian-based digest had 51 HIGH and 3 CRITICAL OS package/advisory occurrences with no upstream fixes. Commit `c8287f7` moved to pinned `python:3.12-alpine`, applied available `apk` security updates, and retained non-root execution. On 2026-09-15, Trivy 0.74.0 scanned the currently deployed digest `sha256:901e6c50d63159f65c4bf552d62856d75d9c8c1ff96ddfeb27338ae2dacb41a1` from a local immutable archive and reported zero HIGH/CRITICAL findings across Alpine 3.24.1 and all detected Python packages; `pip-audit -r requirements.txt` reported no known Python dependency vulnerabilities.
