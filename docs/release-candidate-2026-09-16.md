...
- A controlled outage revision set only the platform endpoint to `https://unreachable.invalid` and the circuit threshold to one. WorkloadApp create returned the expected safe `503 Platform SRE Agent is temporarily unavailable`. The restore completed despite a local Azure CLI connection reset: direct ARM inspection confirmed the latest-ready `p13recovery20260916160503` revision restored the known-good platform endpoint and circuit threshold five; `/health/live`, `/health/ready`, and `/mcp/` then returned `200`. The failed create remains a deliberately retained uncertain outcome pending its bounded no-thread reservation reconciliation.
- The malformed-report fixture experiment verified that the proxy routes its platform start message with the explicit `/agent workload_liaison_p13_malformed_fixture` command. The Azure SRE runtime created the fixture subagent task but left it at `0 completed, 0 failed`, so it could not provide deterministic live fixture output. The temporary fixture agent was removed and normal routing restored. The deterministic HTTP-level regression uses that exact invalid-final Markdown corpus and verifies `502 invalid_platform_response` with no marker, invalid verdict, or report content disclosed.
...
This is a deployable release candidate, not an approved broader rollout. P13.3 has staging evidence for revocation, query injection, idempotency, uncertain creation, rolling restart, outage/recovery, and no-poll reconciler cleanup. Malformed findings are covered by deterministic HTTP-level regression because the Azure SRE custom-agent runtime cannot complete a fixture subagent deterministically. P13.5 requires the named operator/reviewer to approve the finding-by-finding closure after that evidence is recorded.