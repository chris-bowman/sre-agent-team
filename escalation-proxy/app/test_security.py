...
@pytest.mark.asyncio
async def test_status_lookup_for_missing_or_foreign_investigation_is_opaque_not_found(registry):
    investigation_id = registry.create_investigation(
        caller_oid="oid1",
        caller_appid="appid1",
        workload_name="workload-a",
    ...
    with (
        ...
        with pytest.raises(HTTPException) as exc:
            await _get_status_impl(GetInvestigationRequest(investigation_id=investigation_id), foreign_caller)
